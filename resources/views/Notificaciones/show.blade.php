...
<h3>¡Hola {{auth()->user()->name}}! Aquí podrás encontrar las notificaciones de tus equipos y torneos que lideres.</h3>
 @foreach ($notifications as $notification) {{--Notificaciones de representantes (miembros-equipos)--}}
    ...
    El usuario: <b>{{$user->name}}</b> desea ser miembro del equipo <b>"{{$equipo->name}}."</b>
...
@endforeach
@foreach ($notifications as $notification) {{--Notificaciones de organizadores torneos por equipo--}}
    <form action="{{ route('notification.send') }}" method="post">
    @csrf
    @php
    $equipo = App\Models\Equipo::find($notification->equipo_id);
    ...
    El equipo: <b>{{$equipo->name}}</b> desea ser miembro del torneo por equipos <b>{{$torneo->name}}.</b><input type="hidden" name="user_id" value="{{$notification->user_id}}" />
    <input type="hidden" name="equipo_id" value="{{$notification->equipo_id}}" />
...
@endforeach
@foreach ($notifications as $notification) {{--Notificaciones de organizadores torneos individuales--}}
    <form action="{{ route('notification.send') }}" method="post">
    @csrf
    @php
    $user = App\Models\User::find($notification->user_id);
    ...
    El participante: <b>{{$user->name}}</b> desea ser miembro del torneo individual <b>{{$torneo->name}}.</b>
    <input type="hidden" name="user_id" value="{{$notification->user_id}}" />
    <input type="hidden" name="torneo_id" value="{{$notification->torneo_id}}" />
    <button type="submit" name="action" value="aceptada">Aceptar</button>
    <button type="submit" name="action" value="rechazada">Rechazar</button>
...
@endforeach
@foreach ($notifications as $notification) {{--Notificaciones de usuario (participante)--}}
    <form action="{{ route('notification.send') }}" method="post">
    @csrf
    @php
    $user = App\Models\User::find($notification->user_id);
    ...
    Tú solicitud fue aceptada <b>{{$user->name}}</b> y ahora eres participante del torneo <b>{{$torneo->name}}.</b>
    <input type="hidden" name="user_id" value="{{$notification->user_id}}" />
    <input type="hidden" name="torneo_id" value="{{$notification->torneo_id}}" />
...
@endforeach
@foreach ($notifications as $notification) {{--Notificaciones de usuario (equipo)--}}
    <form action="{{ route('notification.send') }}" method="post">
    @csrf
    @php
    $equipo = App\Models\Equipo::find($notification->equipo_id);
    ...
    Tú solicitud de equipo fue aceptada <b>{{$user->name}}</b> y ahora tú equipo <b>{{$equipo->name}}</b> es miembro del torneo <b>{{$torneo->name}}.</b>
    <input type="hidden" name="user_id" value="{{$notification->user_id}}" />
    <input type="hidden" name="equipo_id" value="{{$notification->equipo_id}}" />
    <input type="hidden" name="torneo_id" value="{{$notification->torneo_id}}" />
    <button type="submit" name="action" value="rechazada">Aceptar</button>
...
@endforeach
@foreach ($notifications as $notification) {{--Notificaciones de usuario (miembro)--}}
    <form action="{{ route('notification.send') }}" method="post">
    @csrf
    @php
    $equipo = App\Models\Equipo::find($notification->equipo_id);
    ...
    Tú solicitud fue aceptada <b>{{$user->name}}</b> y ahora eres miembro del equipo <b>{{$equipo->name}}</b>
    <input type="hidden" name="user_id" value="{{$notification->user_id}}" />
    <input type="hidden" name="equipo_id" value="{{$notification->equipo_id}}" />
    <button type="submit" name="action" value="rechazada">Aceptar</button>
    <br> 
